...
pub async fn autoconfigure_links(data_dir: &Path, app_id: &str) -> Result<(), AppError> {
    ...
    for link in &state.app_links {
        ...
        match link_type {
            ...
            "download_client" => match source_def.as_str() {
                "sonarr" => {
                    if let Err(e) =
            ...
            "app_sync" => match source_def.as_str() {
                "prowlarr" => {
                ...
                other => {
                    warn!(
                        "autoconfigure: '{app_id}' (type {other}) has 'app_sync' link \
                         but is not prowlarr — skipping"
                    );
                }
            },

            // ── Sonarr/Radarr → Prowlarr (reverse direction) ────────────────
            "indexer_sync" => match target_def.as_str() {
                "prowlarr" => {
                    if let Err(e) = configure_prowlarr_app_sync(
                        data_dir,
                        target_id,
                ...
                other => {
                    warn!(
                        "autoconfigure: '{app_id}' has 'indexer_sync' link to \
                         '{target_id}' (type {other}), but target is not prowlarr \
                         — skipping"
                    );
                }
            },
            ...
            unknown => {
                warn!(
                    "autoconfigure: unknown link_type '{unknown}' for \
                     {app_id} → {target_id} — skipping"
                );
            }
        }
    }
        ...
        for (other_id, other_state) in &all {
            ...
            for link in &other_state.app_links {
                if link.app_id == app_id
                    && (link.link_type == "indexer_sync" || link.link_type == "app_sync")
                {
                    if let Err(e) =
...
pub async fn autoconfigure_all_linked(data_dir: &Path, app_id: &str) -> Result<(), AppError> {
    ...
    for (other_id, other_state) in &all {
        ...
        let other_links_to_us = other_state.app_links.iter().any(|l| l.app_id == app_id);

        // app_id has a link pointing TO other_id.
        let we_link_to_other = app_state.app_links.iter().any(|l| l.app_id == *other_id);
